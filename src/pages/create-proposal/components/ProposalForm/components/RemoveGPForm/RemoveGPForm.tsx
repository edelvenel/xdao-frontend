...
export function RemoveGPForm({ data, onResponse }: IRemoveGPFormProps) {
	const [name, setName] = React.useState<string>('');
	const [description, setDescription] = React.useState<string>('');
	const [votingDuration, setVotingDuration] = React.useState<number | null>(null);
	...
	}, [createProposal, description, gpToRemove, name, onResponse, votingDuration]);

	React.useEffect(() => {
		if (data !== null) {
			setGpToRemove(data.gpToRemove);
		}
	}, [data]);
			...
			<div className={css.form}>
				<div className={css.fields}>
					<div className={css.block}>
						...
						/>
						<VotingDuration value={votingDuration} setValue={setVotingDuration} />
						<Dropdown
							placeholder="Select GP to remove"
							onSelect={setGpToRemove}
							options={['ox12345...wejnr3r', 'ox...123456789', 'ox...123456790', 'ox...123456791']}
							selected={gpToRemove}
						/>
					</div>
					<div className={css.block}>
